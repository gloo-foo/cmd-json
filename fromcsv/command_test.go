...
func TestFromCsv_Basic(t *testing.T) {
	in := "name,age,city\nAlice,30,NYC\nBob,25,LA\n"
	got, err := testable.TestLines(fromcsv.FromCsv(), run.Input(in))
	if err != nil {
		t.Fatal(err)
	}
	eq(t, got, []string{
...
func TestFromCsv_NoHeader(t *testing.T) {
	in := "Alice,30,NYC\nBob,25,LA\n"
	got, err := testable.TestLines(fromcsv.FromCsv(fromcsv.FromCSVWithoutHeader), run.Input(in))
	if err != nil {
		t.Fatal(err)
	}
	eq(t, got, []string{
...
func TestFromCsv_CustomDelimiter(t *testing.T) {
	in := "name|age\nAlice|30\n"
	got, err := testable.TestLines(fromcsv.FromCsv(fromcsv.Delimiter('|')), run.Input(in))
	if err != nil {
		t.Fatal(err)
	}
...
func TestFromCsv_Trim(t *testing.T) {
	...
	got, err := testable.TestLines(fromcsv.FromCsv(fromcsv.FromCSVTrim), "name, age\nAlice,  30\n")
	if err != nil {
		t.Fatal(err)
	}
	eq(t, got, []string{`{"age":"30","name":"Alice"}`})
