...
func ExampleFromCsv() {
	in := "name,age,city\nAlice,30,NYC\nBob,25,LA\nCharlie,35,London\n"
	lines, _ := testable.TestLines(fromcsv.FromCsv(), run.Input(in))
...
func ExampleFromCsv_noHeader() {
	in := "Alice,30,NYC\nBob,25,LA\n"
	lines, _ := testable.TestLines(fromcsv.FromCsv(fromcsv.FromCSVWithoutHeader), run.Input(in))
	for _, line := range lines {
		fmt.Println(line)
	}
	// Output:
...
func ExampleFromCsv_customDelimiter() {
	in := "name|age|city\nAlice|30|NYC\nBob|25|LA\n"
	lines, _ := testable.TestLines(fromcsv.FromCsv(fromcsv.Delimiter('|')), run.Input(in))
	for _, line := range lines {
		fmt.Println(line)
	}
	// Output:
