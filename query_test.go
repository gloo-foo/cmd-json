...
func TestQuery_FilterMapSortLimit(t *testing.T) {
	in := `{"name":"a","stars":500}` + "\n" +
		`{"name":"b","stars":3000}` + "\n" +
		`{"name":"c","stars":2000}` + "\n"
	q := `filter .stars > 1000 | map { name: .name, stars: .stars } | sort .stars desc | limit 2`
	lines, err := testable.TestLines(command.Query(q), in)
	if err != nil {
		t.Fatal(err)
	}
...
func TestQuery_TopLevelArrayInput(t *testing.T) {
	in := `[{"n":1},{"n":2},{"n":3}]`
	lines, err := testable.TestLines(command.Query(`filter .n >= 2`), in)
	if err != nil {
		t.Fatal(err)
	}
	if len(lines) != 2 {
...
func TestQuery_Reduce(t *testing.T) {
	in := `{"v":2}` + "\n" + `{"v":4}` + "\n"
	lines, err := testable.TestLines(command.Query(`reduce sum(.v)`), in)
	if err != nil {
		t.Fatal(err)
	}
...
func TestQuery_RunErrorFailsStream(t *testing.T) {
	in := `{"n":"x"}` + "\n" + `{"n":1}` + "\n"
	if _, err := testable.TestLines(command.Query(`sort .n`), in); err == nil {
