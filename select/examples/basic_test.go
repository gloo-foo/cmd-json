...
func ExampleSelect_hasField() {
	in := `{"name":"Alice","age":30}` + "\n" +
		`{"name":"Bob"}` + "\n" +
		`{"name":"Charlie","age":35}` + "\n"
	lines, _ := testable.TestLines(selectcmd.Select(selectcmd.HasField("age")), run.Input(in))
...
func ExampleSelect_fieldEquals() {
	...
	lines, _ := testable.TestLines(selectcmd.Select(selectcmd.FieldEquals("status", "active")), run.Input(in))
	for _, line := range lines {
		fmt.Println(line)
	}
	// Output:
...
func ExampleSelect_and() {
	...
	lines, _ := testable.TestLines(selectcmd.Select(cond), run.Input(in))
	for _, line := range lines {
		fmt.Println(line)
	}
	// Output:
