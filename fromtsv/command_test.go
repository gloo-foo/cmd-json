...
func TestFromTsv_Basic(t *testing.T) {
	in := "name\tage\tcity\nAlice\t30\tNYC\nBob\t25\tLA\n"
	got, err := testable.TestLines(fromtsv.FromTsv(), run.Input(in))
	if err != nil {
		t.Fatal(err)
	}
	eq(t, got, []string{
...
func TestFromTsv_NoHeader(t *testing.T) {
	in := "Alice\t30\tNYC\nBob\t25\tLA\n"
	got, err := testable.TestLines(fromtsv.FromTsv(fromtsv.FromTSVWithoutHeader), run.Input(in))
	if err != nil {
		t.Fatal(err)
	}
	eq(t, got, []string{
...
func TestFromTsv_Trim(t *testing.T) {
	...
	got, err := testable.TestLines(fromtsv.FromTsv(fromtsv.FromTSVTrim), "name\tage\nAlice\t  30\n")
	if err != nil {
		t.Fatal(err)
	}
